...
{
    public partial class AdvanceAdministrationViewModel : ValidationViewModelBase
    {
        public AdvanceAdministrationViewModel()
            : base()
        {
            ReloadGroups();
        }
        ...
        public ObservableCollection<SysClass> SelectedClasses { set; get; }
        ...
        public static ObservableCollection<AdminGroupClass> GetClassesItems(int idGroup)
        {
            var classes = COSContext.Current.SysClasses.ToList();

            return GetClasses(null, classes, idGroup);
        }

        static ObservableCollection<AdminGroupClass> privateClasses = new ObservableCollection<AdminGroupClass>();
        public static ObservableCollection<AdminGroupClass> GetClasses(SysClass currentClass, List<SysClass> classes, int groupId)
        {
            ObservableCollection<AdminGroupClass> result = new ObservableCollection<AdminGroupClass>();

            AdminGroupClass newItem = null;
            if (currentClass != null)
            {
                foreach (var itm in classes.Where(a => a.ID_Parent == currentClass.ID))
                {
                    newItem = new AdminGroupClass(itm, groupId);
                    if (!privateClasses.Contains(newItem))
                        privateClasses.Add(newItem);
                    newItem.ParentItem = privateClasses.FirstOrDefault(a => a.SystemClass == currentClass);
                    newItem.RelatedItems = GetClasses(itm, classes, groupId);
                    result.Add(newItem);
                }
            }
            else
            {
                foreach (var itm in classes.Where(a => a.ID_Parent == null))
                {
                    newItem = new AdminGroupClass(itm, groupId);
                    if (!privateClasses.Contains(newItem))
                        privateClasses.Add(newItem);
    ...
    public class AdminGroupClass : NotifyBase
    {
        public AdminGroupClass(SysClass cls, int idGroup)
        {
            RelatedItems = new ObservableCollection<AdminGroupClass>();

            this.SystemClass = cls;
            this.GroupId = idGroup;

            this.LocalEgps = new ObservableCollection<SysEGP>();

            foreach (var itm in cls.t_sys_egp.Where(a => a.ID_sys_group == idGroup).ToList().OrderBy(a => a.Action.Name))
                this.LocalEgps.Add(itm);

        }

        public void RefreshEgps()
        {
            LocalEgps.Clear();

            foreach (var itm in this.SystemClass.t_sys_egp.Where(a => a.ID_sys_group == this.GroupId).ToList().OrderBy(a => a.Action.Name))
                this.LocalEgps.Add(itm);
        }

        public SysClass SystemClass { set; get; }
        public int GroupId { set; get; }

        
        private ObservableCollection<SysEGP> _LocalEgps = new ObservableCollection<SysEGP>();
        public ObservableCollection<SysEGP> LocalEgps
        {
            get { return _LocalEgps; }
            set
            {
                _LocalEgps = value;
                OnPropertyChanged("LocalEgps");
            }
        }

        public AdminGroupClass ParentItem { set; get; }

        private ObservableCollection<AdminGroupClass> _relatedItems = new ObservableCollection<AdminGroupClass>();
        public ObservableCollection<AdminGroupClass> RelatedItems
        {
