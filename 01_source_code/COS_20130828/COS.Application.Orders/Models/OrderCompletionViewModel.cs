...
{
    public partial class OrderCompletionViewModel : ValidationViewModelBase
    {
        ...
        private void AcceptedOrder(Order order)
        {
            if (order != null)
            {
                if (!order.AcceptedDate.HasValue)
                {
                    order.AcceptedBy = COSContext.Current.CurrentUser;
                    order.AcceptedDate = DateTime.Today;
        ...
        private DateTime? _selectedDateFrom = DateTime.Today.AddDays(-5);
        ...
        public void InitOrders(IEnumerable<Order> orders)
        {
            ...
            foreach (var itm in orders.OrderByDescending(a => a.CreateDate))
                LocalOrders.Add(itm);
        }
        ...
        public void RefreshOrders()
        {
            ...
            var myDepartments = COSContext.Current.OrderDepartmentUsers.Where(a => a.ID_user == COSContext.Current.CurrentUser.ID).Select(a => a.Department).Select(a => a.ID);
            COSContext.Current.Refresh(System.Data.Objects.RefreshMode.StoreWins, COSContext.Current.Orders.Where(a => myDepartments.Contains(a.ID_department) && a.ApproveDate != null && a.RejectedDate == null));

            var orders = COSContext.Current.Orders.Where(a => myDepartments.Contains(a.ID_department) && a.ApproveDate != null && a.RejectedDate == null);
