...
{
    public partial class OrderInputlViewModel : ValidationViewModelBase
    {
        public OrderInputlViewModel()
            : base()
        {
            RefreshOrders();
        ...
        public void Save()
        {
            ...
            if (SelectedItem != null)
            {
                ...
                if (string.IsNullOrEmpty(Error) && string.IsNullOrEmpty(customErrors))
                {
                    if (SelectedItem.ID == 0)
                    {
                        var cosc = COSContext.Current;

                        SelectedItem.CreateDate = DateTime.Today;
                        SelectedItem.CreateByID = cosc.CurrentUser.ID;
                        SelectedItem.Status = cosc.OrderStatus.FirstOrDefault(a => a.Code == "N");

                        foreach (var itm in SelectedItemDetails)
                        {
                            itm.Status = SelectedItem.Status;
                            SelectedItem.OrderDetails.Add(itm);
                            cosc.OrderDetails.AddObject(itm);
                        }

                        cosc.Orders.AddObject(SelectedItem);
                    }
                    else
                    {
