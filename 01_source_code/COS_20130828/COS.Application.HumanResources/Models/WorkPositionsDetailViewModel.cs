...
{
    public partial class WorkPositionsDetailViewModel : ValidationViewModelBase
    {
        public WorkPositionsDetailViewModel()
            : base()
        {

        }

        private WorkPosition _selectedItem = null;

        public WorkPosition SelectedItem
        {
            set
            {
                _selectedItem = value;
        ...
        public void Save()
        {
                ...
                if (string.IsNullOrEmpty(Error) && string.IsNullOrEmpty(customErrors))
                {
                    ...
                    {
                        COSContext.Current.WorkPositions.AddObject(SelectedItem);
                    }
                    else
                    {
