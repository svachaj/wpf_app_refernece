...
{
    public partial class SalaryGroupsDetailViewModel : ValidationViewModelBase
    {
        public SalaryGroupsDetailViewModel()
            : base()
        {

        }

        private SalaryGroup _selectedItem = null;

        public SalaryGroup SelectedItem
        {
            set
            {
                _selectedItem = value;
        ...
        public void Save()
        {
                ...
                if (string.IsNullOrEmpty(Error) && string.IsNullOrEmpty(customErrors))
                {
                    ...
                    {
                        COSContext.Current.SalaryGroups.AddObject(SelectedItem);
                    }
                    else
                    {
        ...
        public string IsValid()
        {
            ...
            {
                err += ResourceHelper.GetResource<string>("m_Body_HR00000002");
                err += Environment.NewLine;
            }

            if (string.IsNullOrEmpty(SelectedItem.ID_RON))
            {
                err += ResourceHelper.GetResource<string>("m_Body_HR00000003");
                err += Environment.NewLine;
            }
