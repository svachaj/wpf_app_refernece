...
{
    public partial class WorkCentersDetailViewModel : ValidationViewModelBase
    {
        public WorkCentersDetailViewModel()
            : base()
        {

        }

        private WorkCenter _selectedItem = null;
        public WorkCenter SelectedItem
        {
            set
            {
                _selectedItem = value;
        ...
        public void Save()
        {
                ...
                if (string.IsNullOrEmpty(Error) && string.IsNullOrEmpty(customErrors))
                {
                    ...
                    {
                        COSContext.Current.WorkCenters.AddObject(SelectedItem);
                    }
                    else
                    {
        ...
        public string IsValid()
        {
            ...
            {
                err += ResourceHelper.GetResource<string>("m_Body_ENG00000001");
                err += Environment.NewLine;
            }

            if (string.IsNullOrEmpty(SelectedItem.Value))
            {
                err += ResourceHelper.GetResource<string>("m_Body_ENG00000013");
                err += Environment.NewLine;
            }
