...
{
    public partial class KPIWcsReportViewModel : ValidationViewModelBase
    {
        public KPIWcsReportViewModel()
            : base()
        {
        ...
        void KPIReportViewModel_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            if (e.PropertyName == "SelectedDivision")
            {
                ...
                {
                    LocalWorkGroups = CurrentContext.WorkGroups.Where(a => a.ID_Division == SelectedDivision.ID).ToList();
                }
            }
            else if (e.PropertyName == "SelectedWorkGroup")
            {
                if (SelectedWorkGroup != null)
                    LocalWorkCenters = CurrentContext.WorkGroupsWorkCenters.Where(a => a.ID_WorkGroup == SelectedWorkGroup.ID).Select(a => a.WorkCenter).ToList();
            }
        }
        ...
        public void ShowReportInternal()
        {
            ...
            if (IsUsedWorkCenters)
            {
                ...
                {
                    var wgs = SelectedDivision != null ? CurrentContext.WorkCenters.Where(a => a.ID_Division == SelectedDivision.ID).ToList() : CurrentContext.WorkCenters.ToList();
                    var sets = CurrentContext.UserProperties.FirstOrDefault(a => a.ID_user == null && a.Key == "KPIReportFilterUsedWCS");
            ...
            if (IsUsedWorkGroups)
            {
                ...
                {
                    var wgs = SelectedDivision != null ? CurrentContext.WorkGroups.Where(a => a.ID_Division == SelectedDivision.ID).ToList() : CurrentContext.WorkGroups.ToList();
                    var sets = CurrentContext.UserProperties.FirstOrDefault(a => a.ID_user == null && a.Key == "KPIReportFilterUsedWGS");
            ...
            ReportData = new List<KPIReportData>();
            KPIReportData data = null;

            List<WorkCenter> wcs = CurrentContext.WorkGroupsWorkCenters.Where(a => a.ID_WorkGroup == SelectedWorkGroup.ID).Select(a => a.WorkCenter).ToList();
            ...
            switch (SelectedShowBy)
            {
                case ShowBy.All:
                    ...
                    {
                        data = new KPIReportData(wcs);
                        data.IdDivision = itm.ID_Division;

                        data.KpiActualTimeDict[itm.WorkCenter.Value] = itm.ActualTime_min;
                        data.KpiProducedPcsDict[itm.WorkCenter.Value] = itm.ProducedPieces;
                        data.ScrapPcsDict[itm.WorkCenter.Value] = itm.ScrapPieces;
                        data.HlpPerformanceDict[itm.WorkCenter.Value] = itm.hlpPerformance;
                        data.HlpProductivityDict[itm.WorkCenter.Value] = itm.hlpHrProductivity;
                        data.ActualPcsPerHeadhourDict[itm.WorkCenter.Value] = itm.ActPiecesPerHeadHour;
                        data.OperationalTimeDict[itm.WorkCenter.Value] = itm.ActOperationalTime_min;
                ...
                case ShowBy.Day:
                    ...
                    foreach (var itm in prods)
                    {
                        ...
                        KPIReportData existed = ReportData.FirstOrDefault(a => a.Day == dayofyear && a.Year == year);

                        if (existed != null)
                        {
                            existed.ItemsCountDict[itm.WorkCenter.Value]++;
                            existed.KpiActualTimeDict[itm.WorkCenter.Value] += itm.ActualTime_min;
                            existed.KpiProducedPcsDict[itm.WorkCenter.Value] += itm.ProducedPieces;
                            existed.ScrapPcsDict[itm.WorkCenter.Value] += itm.ScrapPieces;
                            existed.HlpPerformanceDict[itm.WorkCenter.Value] += itm.hlpPerformance;
                            existed.HlpProductivityDict[itm.WorkCenter.Value] += itm.hlpHrProductivity;
                            existed.ActualPcsPerHeadhourDict[itm.WorkCenter.Value] += itm.ActPiecesPerHeadHour;
                            existed.OperationalTimeDict[itm.WorkCenter.Value] += itm.ActOperationalTime_min;
                ...
                case ShowBy.Month:
                    ...
                    foreach (var itm in prods)
                    {
                        ...
                        int year = itm.Date.Year;
                        KPIReportData existed = ReportData.FirstOrDefault(a => a.Month == month && a.Year == year);
                ...
                case ShowBy.Week:
                    ...
                    {
                        int year = itm.Date.Year;
                        KPIReportData existed = ReportData.FirstOrDefault(a => a.Week == itm.Week && a.Year == year);
                ...
                case ShowBy.Year:
                    ...
                    foreach (var itm in prods)
                    {
                        KPIReportData existed = ReportData.FirstOrDefault(a => a.Week == itm.Date.Year);
        ...
        public decimal SumActPiecesPerHeadHourOperationalTime
        {
            get
            {
                return OperationalTime == 0 ? 0 : ActualPcsPerHeadhour / ReportData.Sum(a => a.ItemsCount) * OperationalTime;
            }
        }
        ...
        {
            foreach (var itm in ReportData)
            {
                foreach (var ki in itm.ActualPcsPerHeadhourDict.Keys)
                {
                    itm.Add<decimal>("KpiPerformance" + ki, itm.KpiPerformanceC(ki));
                    itm.Add<decimal>("KpiQuality" + ki, itm.KpiQualityC(ki));
                    itm.Add<decimal>("KpiProductivity" + ki, itm.KpiProductivityC(ki));
                    itm.Add<decimal>("KpiAvailability" + ki, itm.KpiAvailabilityC(ki));
                    itm.Add<decimal>("KpiOEE" + ki, itm.KpiOEEC(ki));
                }
            }
        }
