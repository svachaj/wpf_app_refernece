...
{
    public partial class KPIReportViewModel : ValidationViewModelBase
    {
        ...
        public void ShowReportInternal()
        {
            ...
            if (IsUsedWorkCenters)
            {
                if (UsedWorkCenters == null)
                {
                    ...
                    var sets = CurrentContext.UserProperties.FirstOrDefault(a => a.ID_user == null && a.Key == "KPIReportFilterUsedWCS" + DivisionParam);
                ...
                var idsList = UsedWorkCenters.Select(a => a.ID).ToList();

                prods = prods.Where(a => idsList.Contains(a.ID_WorkCenter)).ToList();
            }
            ...
            if (IsUsedWorkGroups)
            {
                if (UsedWorkGroups == null)
                {
                    ...
                    var sets = CurrentContext.UserProperties.FirstOrDefault(a => a.ID_user == null && a.Key == "KPIReportFilterUsedWGS" + DivisionParam);
                ...
                var idsList = UsedWorkGroups.Select(a => a.ID).ToList();

                prods = prods.Where(a => idsList.Contains(a.ID_WorkGroup)).ToList();
            }
            ...
            ActualPcsPerHeadhourOperTime = 0;
            ...
            switch (SelectedShowBy)
            {
                case ShowBy.All:
                    ...
                    {
                        KpiActualTime += itm.ActualTime_min;
                        KpiProducedPcs += itm.ProducedPieces;
                        ScrapPcs += itm.ScrapPieces;
                        HlpPerformance += itm.hlpPerformance;
                        HlpProductivity += itm.hlpHrProductivity;
                        ActualPcsPerHeadhour += itm.ActPiecesPerHeadHour;
                        OperationalTime += itm.ActOperationalTime_min;
                        ActualPcsPerHeadhourOperTime += itm.ActPiecesPerHeadHour * itm.ActOperationalTime_min;


                        KPIReportData existed = ReportData.FirstOrDefault(a => a.Date == itm.Date);
        ...
        public decimal SumActPiecesPerHeadHourOperationalTime
        {
            get
            {
                ...
                // return OperationalTime == 0 ? 0 : ActualPcsPerHeadhour  * OperationalTime;
            }
        }
