...
{
    public partial class LogisticDomesticForwarderReportViewModel : ValidationViewModelBase
    {
        public LogisticDomesticForwarderReportViewModel(COSContext dataContext)
            : base()
        {
        ...
        public void ShowReportInternal()
        {
            ...
            var logistics = this.dataContext.DomesticExportDetails.AsQueryable();

            if (SelectedCarType != null)
                logistics = logistics.Where(a => a.Export.ID_carType == SelectedCarType.ID);

            if (SelectedCostCenter != null)
                logistics = logistics.Where(a => a.ID_costCenter == SelectedCostCenter.ID);

            if (SelectedCountry != null)
                logistics = logistics.Where(a => a.Destination.ID_country == SelectedCountry.ID);

            if (SelectedCustomer != null)
                logistics = logistics.Where(a => a.ID_customer == SelectedCustomer.ID);

            if (SelectedPointOfOrigin != null)
                logistics = logistics.Where(a => a.Export.ID_pointOfOrigin == SelectedPointOfOrigin.ID);

            if (!SelectedSoNumber.IsNullOrEmptyString())
                logistics = logistics.Where(a => a.TrpSos.Where(x => x.SO == SelectedSoNumber).Count() > 0);

            if (SelectedForwarder != null)
                logistics = logistics.Where(a => a.Export.ID_forwarder == SelectedForwarder.ID);

            if (SelectedDestination != null)
                logistics = logistics.Where(a => a.ID_destination == SelectedDestination.ID);
            ...
            if (SelectedDateFrom.HasValue)
                logistics = logistics.Where(a => a.Export.PlannedDate >= SelectedDateFrom);

            if (SelectedDateTo.HasValue)
                logistics = logistics.Where(a => a.Export.PlannedDate <= SelectedDateTo);

            logistics = logistics.OrderBy(a => a.Export.PlannedDate);
            ...
            Models = new List<LogisticDomesticForwarderReportViewModel>();

            Models.Add(this);
        }

        public List<LogisticDomesticForwarderReportViewModel> Models = null;
        ...
        private List<DomesticExportDetail> _reportData = null;
        public List<DomesticExportDetail> ReportData
        {
            set
            {
                _reportData = value;
