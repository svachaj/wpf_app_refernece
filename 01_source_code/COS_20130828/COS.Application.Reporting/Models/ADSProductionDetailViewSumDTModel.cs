...
{
    public partial class ADSProductionDetailViewSumDTModel : ValidationViewModelBase
    {
        public ADSProductionDetailViewSumDTModel()
            : base()
        {
        ...
        public void ShowReportInternal()
        {
            ...
            KpiActualTime = prods.Sum(a => a.ActualTime_min);
            KpiProducedPcs = prods.Sum(a => a.ProducedPieces);
            ScrapPcs = prods.Sum(a => a.ScrapPieces);
            HlpPerformance = prods.Sum(a => a.hlpPerformance);
            HlpProductivity = prods.Sum(a => a.hlpHrProductivity);
            ActualPcsPerHeadhour = prods.Sum(a => a.ActPiecesPerHeadHour);
            OperationalTime = prods.Sum(a => a.ActOperationalTime_min);
            PlannedPcs = prods.Sum(a => a.StdPiecesPerHour);
            KpiDowntimeTime = prods.Sum(a => a.DowntimeTime_min);
            HrDowntimeTime = prods.Sum(a => a.HrDowntimeTime_min);
            ScrapCountedWeight = prods.Sum(a => a.ScrapCountedWeigh);
            ...
            {                
                var exist = DowntimesSums.FirstOrDefault(a => a.Description == ast.Downtime.Description);
                if (exist != null)
                {
                ...
                {
                    DowntimesSums.Add(new DowntimeSum() { Description = ast.Downtime.Description, Time = ast.Time_min, Rate = 1 });
                }
            }

            DowntimesSums.Add(new DowntimeSum() { Description = ResourceHelper.GetResource<string>("rep_SetupInStd"), Time = prods.Sum(a => a.hlpInSetupTime), Rate = prods.Where(a => a.hlpInSetupTime != 0).Count() });
            DowntimesSums.Add(new DowntimeSum() { Description = ResourceHelper.GetResource<string>("rep_SetupOutStd"), Time = prods.Sum(a => a.hlpOutSetupTime), Rate = prods.Where(a => a.hlpOutSetupTime != 0).Count() });


            Models = new List<ADSProductionDetailViewSumDTModel>();
        ...
        private List<HourlyProduction> _reportData = null;
        public List<HourlyProduction> ReportData
        {
            set
            {
                _reportData = value;
    ...
    public class DowntimeSum
    {
        ...
        public string DescriptionHide { set; get; }
        public decimal Time { set; get; }
        public int Rate { set; get; }
        public decimal ActualTime { set; get; }
        public decimal Weight 
        {
            get 
            {
                if (Time > 0)
                    return   (Time / ActualTime) * 100;
                else
                    return 0;
            }
        }
