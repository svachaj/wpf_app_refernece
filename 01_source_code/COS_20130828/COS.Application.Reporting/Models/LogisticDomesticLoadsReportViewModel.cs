...
{
    public partial class LogisticDomesticLoadsReportViewModel : ValidationViewModelBase
    {
        public LogisticDomesticLoadsReportViewModel(COSContext dataContext)
            : base()
        {
        ...
        public void ShowReportInternal()
        {
            ...
            ForwarderPrice = ReportData.Sum(a => a.ExportDetails.Sum( c => c.ForwarderPrice ));

            BafPrice = ReportData.Sum(a => a.ExportDetails.Sum( c => c.BafPrice ));
            TotalPrice = ReportData.Sum(a => a.ExportDetails.Sum( c => c.TotalPrice ));
            VolumeCBM = ReportData.Sum(a => a.ExportDetails.Sum( c => c.VolumeCBM ));
            TransportCount = ReportData.Count;

            Models = new List<LogisticDomesticLoadsReportViewModel>();

            Models.Add(this);
        }

        public List<LogisticDomesticLoadsReportViewModel> Models = null;
